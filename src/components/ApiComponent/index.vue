...
import {cloneDeep, isEqual,get,isFunction} from "lodash-es";
...
const getOptions = computed(() => {
  ...
  function transformData(data: OptionsItem[]): OptionsItem[] {
    return data.map((item) => {
      ...
      const disabled = get(item, disabledKey);
      return {
        ...objectOmit(item, [labelKey, valueKey, disabled, childrenKey]),
        label: get(item, labelKey),
        value: numberToString ? `${value}` : value,
        disabled: get(item, disabledKey),
        ...(childrenKey && item[childrenKey]
          ? { children: transformData(item[childrenKey]) }
          : {}),
      };
    });
  }
...
async function fetchApi() {
  const { api, beforeFetch, afterFetch, resultKey } = props;
  ...
  try {
    ...
    if (resultKey) {
      refOptions.value = get(res, resultKey) || [];
    }
    emitChange();
  } catch (error) {
    console.warn(error);
...
<template>
  ...
  >
    ...
    <template v-if="loading" #[loadingSlot]>
      <el-skeleton
        :rows="1"
        animated
        style="width: 100%; height: 32px; --el-skeleton-circle-size: 0"
      />
    </template>
  </component>
</template>
