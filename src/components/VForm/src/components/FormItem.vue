...
import { cloneDeep, debounce, upperFirst } from 'lodash-es'
import {
  type FormActionType,
  type FormProps,
...
import type { Nullable, Recordable } from '@/hooks/vben/types'
...
export default defineComponent ({
  ...
  setup(props, { slots }) {
    ...
    function renderComponent() {
      ...
      const isInput = component && ['Input', 'Slider', 'DateSelect', 'InputNumber', 'RadioButton'].includes (component)
      ...
      const on = {
        [eventKey]: debounce((...args: Nullable<Recordable<any>>[]) => {
          const [e] = args
          if (propsData[eventKey])
            propsData[eventKey](...args)

          const oldValue = props.formModel[field]
          ...
          if (value !== oldValue)
            props.setFormModel(field, value, props.schema)
        }, 0), // Adjust the delay (in milliseconds) based on your needs
      }
      ...
      const bindValue: Recordable<any> = {
        [valueField || (isInput || isChange ? 'modelValue' : 'value')]: props.formModel[field],
      }
