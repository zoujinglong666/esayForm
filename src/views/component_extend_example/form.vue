...
import { onMounted, ref, WatchOptions ,reactive} from 'vue'
import { ElMessage } from 'element-plus'
import { cloneDeep, set } from 'lodash-es'
import dayjs from 'dayjs'
import type { PlusColumn, FieldValueType, CellContext } from '@/components/PlusTable/types'
...
const tableConfig = ref<PlusColumn[]>([
  {
    label: '姓名',
    prop: 'name',
    formItemProps: (value: FieldValueType, ctx: CellContext) => ({
      rules: [
        { required: true, message: '请输入姓名', trigger: 'change' } as any,
        ...(ctx.row.status === '0'
          ? [{ min: 2, message: '紧急事项姓名不能少于2字符', trigger: 'change' } as any]
          : [])
      ]
    })
  },
    ...
    formItemProps: (value: FieldValueType, ctx: CellContext) => ({
      rules: [
        ...
        ...(ctx.row.name === 'admin'
          ? [
              {
                validator: (rule: any, val: any) => val !== '0',
                message: '管理员不能有未解决状态'
              } as any
            ]
          : [])
  ...
  },
  {
    ...
    formProps: (value: FieldValueType, ctx: CellContext) => ({
      rules: [
        {
          required: ctx.row.status !== '1',
          message: '非已解决状态必须选择日期',
          trigger: 'change'
        } as any
      ]
    }),
  ...
  {
    label: (value: FieldValueType, ctx: CellContext) => ctx.row.name === 'test'?'youxainji':'1111',
...
<!--import type { PlusColumn, FieldValueType, CellContext, OptionsRow } from '@/components/PlusTable/types'-->
...
<!--    width: 120,-->
<!--    valueType: 'select',-->
<!--    options: (value: FieldValueType, ctx: CellContext): OptionsRow[] => {-->
<!--      if (ctx.row.name === 'admin') {-->
...
<!--    },-->
<!--    fieldProps: (value: FieldValueType, ctx: CellContext) => ({-->
<!--      disabled: ctx.row.name === 'Jerry',-->
<!--    }),-->

<!--    editable: (value: FieldValueType, ctx: CellContext) => {-->
<!--      return ctx.row.name === 'admin'-->
<!--    }-->
<!--  },-->
<!--  {-->
<!--    label: '日期',-->
...
<!--    valueType: 'date-picker',-->
<!--    fieldProps: (value: FieldValueType, ctx: CellContext) => ({-->
<!--      disabled: !ctx.row.name-->
<!--    }),-->
<!--    render(value: FieldValueType, data: CellContext) {-->
<!--      return <div>{data.row.name}</div>-->
<!--    },-->
<!--  },-->
<!--  {-->
<!--    label: '分类',-->
